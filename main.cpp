...
{
    CircBuf ab;
    test_(ab.capacity() == 0); //checking initial capacity
    
    ab.insert("ONE", 3); //inserts string into buffer;
    test_(ab.size() == 3); //checking size after insertion
    test_(ab.get(2) == "ON"); //retrieving characters
    test_(ab.size() == 1); //checking size after retrieval
    test_(ab.examine() == "[--E-----]"); //checks if buffer matches expected pattern
